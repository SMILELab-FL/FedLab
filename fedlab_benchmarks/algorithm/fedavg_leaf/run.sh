...
do
  ...
  echo "server started"
  python server_fedavg.py --server_ip 127.0.0.1 --server_port 3002 --world_size ${dataset_world_size[${key}]}  --dataset ${key} &

  for ((i=1; i<${dataset_world_size[$key]}; i++))
  do
  {
      echo "client ${i} started"
      CUDA_VISIBLE_DEVICES=3 python client_fedavg.py --server_ip 127.0.0.1 --server_port 3002 --world_size ${dataset_world_size[${key}]} --local_rank ${i} --dataset ${key}
  } &
  done
  wait
