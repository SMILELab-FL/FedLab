...
serial_handler = SerialTrainer(
    model=local_model, dataset=trainset, data_slices=data_indices, aggregator=aggregator)
...
for round in range(args.com_round):
    ...
    print(selection)
    model_params = serial_handler.train(model_parameters=model_params, epochs=args.epochs,
                                        lr=args.lr, batch_size=args.batch_size, id_list=selection, cuda=True, multi_threading=False)
